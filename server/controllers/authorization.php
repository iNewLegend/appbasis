...
class Authorization
{
     ...
     */
    public function __construct(Core\App $app, Core\Logger $logger, Models\User $user, Models\Attempt $attempt, Models\Session $session)
    {
        $this->logger = $logger;
        ...
        $this->auth = new Services\Auth($this->session, $app->getIp());
    }
    ...
    public function login($email = '', $password = '', $captcha = '', $remember = '')
    {
        ...
        $this->logger->notice("ip: `" . $this->auth->getIp() . "` - Login success");
